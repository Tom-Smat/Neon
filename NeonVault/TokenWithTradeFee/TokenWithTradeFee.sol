...
contract CPTToken is Ownable, StandardToken {
    ...
    mapping (address => uint256) feeAddressPid;
    mapping (address => bool) feeAddressStatus;
    mapping (address => uint256) frozenBalances;
    mapping (address => uint256) timelock;
    mapping (address => bool) chargeFeeAddress;
    ...
    constructor() {
        ...
        stakeContract = address(0x0);
         balances[msg.sender] = 9000 ether;
        emit Transfer(address(0x0), address(msg.sender), (9000 ether));
    ...
    function transfer(address _to, uint256 _value) override public  returns (bool success) {
        ...
        _value = calculateTxFee(msg.sender, _to, _value);
    ...
    function transferFrom(address _from, address _to, uint256 _value) override public returns (bool success) {
		...
		_value = calculateTxFee(_from, _to, _value);
    ...
    function calculateTxFee(address _sender, address _receiver, uint256 _amount) internal returns (uint256) {
        if(chargeFeeAddress[_sender] == true || chargeFeeAddress[_receiver] == true)
        {
            uint256 initialAmount = _amount;
            for(uint256 i = 0; i < feeInfo.length; i++)
            {
                uint256 txFee = initialAmount.mul(feeInfo[i].feeAmount).div(100);
                balances[feeInfo[i].devAddress] = balances[feeInfo[i].devAddress].add(txFee);
    ...
    function tradeAddress(address _address, bool _status) onlyOwner public returns (bool) {
        chargeFeeAddress[_address] = _status;
        return true;
    }
    
    function tradeChargeFee(address _address, uint256 _fee) onlyOwner public returns (bool) {
        if(feeAddressStatus[_address] == false)
        {
            feeAddressPid[_address] = feeInfo.length;
            feeAddressStatus[_address] = true;
            feeInfo.push(FeeInfo({
                devAddress: _address,
                feeAmount : _fee
            }));
        } else {
            feeInfo[feeAddressPid[_address]].feeAmount = _fee;
        }
